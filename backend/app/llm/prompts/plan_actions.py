...
10. 当"可用的 UI 元素"列表提供时，你的 target 描述必须优先使用列表中的已知元素描述（括号中的中文文本）。
11. 不要创建列表中不存在的新元素描述。如果场景步骤提到某个操作但列表中没有对应元素，使用最接近的已知元素，并在 description 中注明差异。
12. target 优先使用已知元素的中文名称，这有助于执行引擎更精确地匹配目标元素。
...
将上述场景步骤按照规划规则翻译为 MCP 工具调用 Action。仅返回 Action 对象的 JSON 数组，不要其他文本。记住：交互工具使用 "target" 参数而不是 "ref"。遵循 UI 元素约束规则，优先使用已知元素描述。"""