...
- browser_drag: 拖拽。args: {"startTarget": "eN", "endTarget": "eN", "startElement": "中文", "endElement": "中文"}
  - 注意：browser_drag 使用 HTML5 拖拽协议，部分看板应用（用 pointer 事件实现拖拽）不响应它。
    若拖拽后元素位置未变，不要反复重试拖拽——改用应用自带的"移动"菜单/对话框完成移动。

## 常见交互模式（重要，避免反复失败）
**可搜索下拉框/组合框选择**（如选 Project、选列表、选标签）：这类控件通常是一个
带搜索输入框的下拉，**仅输入文字不会完成选择**。正确做法分两步：
1. 点击下拉触发器（通常是 "Open dropdown" 按钮）展开选项列表；
2. 选项出现后，**点击列表中那个匹配的选项元素**（它在快照里会有自己的 eN 引用，
   通常是 option/menuitem 角色），而不是只往搜索框里输入文字。
   如果选项太多需要过滤，可以先在搜索框输入文字过滤，但**最后必须点击出现的那个选项**。
- 判断是否已选中：下拉框显示目标值、或下拉关闭，才算选中成功；若仍显示原值，说明没选中，
  换一种方式（直接点选项），不要反复点同一个触发器。
...
## 重要：登录凭据不要猜测
登录由系统在进入循环前自动完成，你不需要主动登录。若发现当前页面是登录页且
显示"Invalid username or password"等错误，说明登录被站点拒绝（通常是限流/凭据过期），
**不要尝试更换用户名/密码重新登录**——这会浪费所有步数。应立即输出 done=true 并在
reasoning 里说明"登录被站点拒绝，无法继续"，让验证/反思环节处理。
