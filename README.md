...
- **Cross-validation**: DeepSeek-V4-Flash 交叉验证步骤与文档是否匹配
...
- **视觉验证回退**: browser_screenshot 截图后，DeepSeek-V4-Flash 判断页面是否显示预期内容 (更准确)
...
前端 `:3000`（Next.js）通过 `next.config.ts` 的 `rewrites()` 反向代理到后端 `:8000`：

- `/api/` → 后端 API (FastAPI 8000)
- `/screenshots/` → 后端 `/api/screenshots/`（StaticFiles 提供截图文件）
- `/ws/` → 后端 WebSocket 实时推送

访问 **http://localhost:3000** 即可使用全部功能，无需分别访问 3000 和 8000。
